...
class Wave(AbstractLayer):
	...
	def __init__(_s, id, pic):
		...
		_s.NUM_REPEATS_OF_SAME_WAVE = 50  # 30 - stops at around 3:30. this times AVG * RAND must be less than frames
		# num_frames_avg * num_repeats_of_same_wave should give
		_s.DEST_RAND_MIN_MAX = [[-62, -0], [-20, 0]]  # 53: 70 first is X, second is Y
	...
	def gen_ld_sss(_s):
		...
		for i in range(len(_s.frame_sss)):  # PER WAVE TRAJECTORY  makes sure each sss outputs different trajectory
			x_rand1 = random.randint(-20, 20)  # both orig and dest  48:
	...
	def gen_extent_alpha_wave(_s, ld_sss):
		...
		for i in range(len(_s.frame_sss)):
			...
			alpha = gen_alpha(_gi, fun_plot='normal', y_range=[0, 0.14])  # 50: 0.15 47: 0.3
			alpha_agg += [0.0]
